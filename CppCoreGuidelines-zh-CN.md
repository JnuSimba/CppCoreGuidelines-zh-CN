...
这里并未表明其意图是“单纯地”循环访问 `v` 的元素。索引的实现细节被暴露了出来（因而可能导致被误用），而且 `i` 的生命周期超出了循环的范围，这有可能是也可能不是设计意图。读者仅从这段代码中是无法了解清楚的。
...
但我们至今还没有能够做到这点的语言设施。
一旦语言支持变为可用（例如，参见[契约提案](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0380r1.pdf)），我们就将会采用前条件，后条件和断言的标准版本。
...
参见 [GOTW #100](https://herbsutter.com/gotw/_100/) 和 [cppreference](https://en.cppreference.com/w/cpp/language/pimpl) 有关这个手法相关的权衡和其他实现细节。
...
[[Str15]](https://www.stroustrup.com/resource-model.pdf) 中对这种例子有所探讨。
...
由此，我们编写这样的类
    ...
    public:
        ...
        Istream() { }
        Istream(czstring p) : owned{true}, inp{new ifstream{p}} {}            // 从文件读取
        Istream(czstring p, Opt) : owned{true}, inp{new istringstream{p}} {}  // 从命令行读取
...
* [F.21: 要返回多个“输出”值，优先返回结构体](#rf-out-multi)
...
##### 示例
    ...
    widget* find(const set<widget>& s, const widget& w, Hint);   // 这里曾经使用过一个形参 hint
...
* 如果函数只是需要为其自己局部使用而保留实参的局部可修改副本，那么按值接受是没问题的。
* 如果函数需要保留参数的一个副本用以传递给另一个目标（其他函数，或存储到非局部位置中），那就在按 `const&` 传递参数（对于左值）之外，
...
* 如果类型的移动操作昂贵（比如 `array<BigTrivial>`），就考虑将其分配在自由存储中并返回一个句柄（比如 `unique_ptr`），或者传递一个指代用以填充的非 `const` 目标对象的引用（将其用作输出参数）。
...
### <a id="rf-out-multi"></a>F.21: 要返回多个“输出”值，优先返回结构体

##### 理由

返回值是自我说明为“仅输出”值的。
注意，C++ 是支持多返回值的，按约定使用的是元组式类型（`struct`、`array`、`tuple` 等），
在调用处使用结构化绑定（C++17）可以获得更多便利。
如果可能则优先使用具名 `struct`。
不过，`tuple` 在变参模板中很有用。
...
C++98 的标准库在一些地方使用了这种风格，一些函数返回的是 `pair`。
...
具有有意义名字的 `struct` 在现代 C++ 中更常见。
例子参见 `ranges::min_max_result`、`from_chars_result` 等其他类型。
...
比较一下，如果所有的值都按返回值传递出来的话，得像如下这样做：
    ...
    get_string_result get_string(istream& in)  // 不建议这样做
    {
        string s;
        in >> s;
        return { in, move(s) };
...
另一种方案是使用 `optional<T>` 或 `expected<T, error_code>` 而非 `pair` 或 `tuple`。
适当使用的话，这些类型比 `pair<T, bool>` or `pair<T, error_code>` 表达更多的有关各成员含义的信息。

##### 注解

当所要返回的对象是从复制操作昂贵的局部变量进行初始化时，
可以用显式 `move` 有效避免复制：
...
* 如果可能，`pair` 或 `tuple` 返回类型应当替换为 `struct`。
  变参模板中通常无法避免 `tuple`。
...
返回局部变量会隐式地移动它。
显式的 `std::move` 总是不良的实践，因为它会阻碍可以把移动完全消除掉的返回值优化（RVO）。
...
此处，一个大型对象（网络消息）被传递给一个迭代算法，而它也许不够高效，或者不能正确复制这个消息（它可能是无法复制的）：
...
当 C++ 带来[统一函数调用](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0251r0.pdf)之后，这条规则会更有效。
...
* [C.33: 如果类中带有有所有权的指针成员，请定义析构函数](#rc-dtor-ptr2)
...
* [C.47: 按成员声明的顺序对数据成员进行定义和初始化](#rc-order)
* [C.48: 对于常量初始化式来说，优先采用默认成员初始化式而不是构造函数中的成员初始化式](#rc-in-class-initializer)
...
* 【复杂】 复制/移动构造函数和对应的复制/移动赋值运算符，应当在相同的解引用层次上向相同的数据成员进行写入。
* 【复杂】 在复制/移动构造函数中被写入的任何数据成员，在其他构造函数中也都应当进行初始化。
* 【复杂】 如果复制/移动构造函数对某个数据成员进行了深复制，就应当在析构函数中对这个数据成员进行修改。
* 【复杂】 如果析构函数修改了某个数据成员，在任何复制/移动构造函数或赋值运算符中就都应当对该数据成员进行写入。
...
* 【简单】 当类中所有的指针或引用成员是所有者
  （比如通过使用 `gsl::owner` 所断定）时，它们就应当在析构函数中有所引用。
* 【困难】 当在所有权上没有明确的说法时，为指针或引用成员确定其是否是所有者
  （比如，检查构造函数的代码）。
...
### <a id="rc-dtor-ptr2"></a>C.33: 如果类中带有有所有权的指针成员，请定义析构函数
...
`Rec2` 的构造函数是多余的。
同样的，`int` 的默认值最好用[默认成员初始化式](#rc-in-class-initializer)来给出。
...
* 【简单】 每个构造函数都应当对每个数据成员进行初始化（明确地，通过委派构造函数调用，或者通过默认构造）。
...
### <a id="rc-default"></a>C.45: 不要定义仅对数据成员进行初始化的默认构造函数；应当使用默认成员初始化式
...
【简单】 若默认构造函数的显式成员初始化式是常量则予以标记，并建议应将该常量改写为数据成员初始化式。
...
### <a id="rc-order"></a>C.47: 按成员声明的顺序对数据成员进行定义和初始化
...
### <a id="rc-in-class-initializer"></a>C.48: 对于常量初始化式来说，优先采用默认成员初始化式而不是构造函数中的成员初始化式
...
* 【简单】 每个构造函数都应该对所有数据成员进行初始化（明确进行，通过委派构造函数调用，或者通过默认构造）。
* 【简单】 构造函数的默认实参的出现表明默认成员初始化式可能更合适。
...
**参见**: 当“重复行为”是简单的初始化时，考虑使用[默认成员初始化式](#rc-in-class-initializer)。
...
如果 `swap` 试图用异常来退出的话，这就是严重的设计错误，程序最好立即终止（terminate）。
...
注意，我们可以为数据成员提供默认初始化式：[C.49: 优先进行初始化而不是在构造函数中赋值](#rc-initialize).
...
比如说，在合适的条件下，是可以以[快速常量时间](https://www.stroustrup.com/fast_dynamic_casting.pdf)来实施 `dynamic_cast` 的。
...
我们打算修正这一点： [Operator Dot (R2)](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2015/n4477.pdf)。
...
现代 C++ 引入了 `std::byte`（C++17）和 `std::bit_cast`（C++20）以支持在原始对象表示上进行的操作。
对于这些操作，应当对 `std::byte` 而非 `unsigned char` 或 `char` 进行 `reinterpret_cast`。
...
也可以考虑使用 `constexpr` 和 `const inline` 变量。
...
除了遭受[资源泄漏](#rp-leak)的问题外，这也带来了一组假性的分配和回收操作，而这其实是不必要的。如果 Gadget 可以廉价地从函数转移出来（就是说，它很小，或者具有高效的移动操作）的话，直接“按值”返回即可（参见[输出返回值](#rf-out)）：
...
这样的话，有可能读者知道 `trim_tail` 是什么意思，而且读者可以找一下它并回忆起来。
...
这个例子经常被当成是用来展示需要更宽松的初始化规则的例子。
    ...
    widget i;    // "widget" 是一个初始化操作昂贵的类型，可能是一种大型平凡类型
    widget j;
...
##### 示例
    ...
    auto zz = gsl::narrow_cast<int>(7.9);  // OK: 这个正是你想要的
...
自 C++17 起，中这段代码没有未定义行为，但仍未指定是哪个实参被首先求值。这个调用会是 `f(1, 2)` 或 `f(2, 1)`，但你不知道是哪个。
...
* 对可以作为[隐式转换](#pro-type-implicitpointercast)的显式指针强制转换进行标记。
...
注意：不要在模板函数中编写量的非依赖的逻辑，因为这会导致代码臃肿。例如，进一步改进是，若 `get_bar_impl` 的全部或部分代码是非依赖代码，将其重构移出到一个公共的非模板函数中，这可能能显著减小生成代码的体积。
...
`std::move()` 是经过伪装的向 `&&` 的强制转换；其自身并不会移动任何东西，但会把具名的对象标记为可被移动的候选者。
...
* 静态强制实施工具：[clang](https://clang.llvm.org/docs/ThreadSafetyAnalysis.html)
和一些老版本的 [GCC](https://gcc.gnu.org/wiki/ThreadSafetyAnnotation)
都提供了一些针对线程安全性性质的静态代码标注。
统一地采用这项技术可以将许多种类的线程安全性错误变为编译时的错误。
这些代码标注一般都是局部的（将某个特定数据成员标记为又一个特定的互斥体进行防护），
且一般都易于学习使用。但与许多的静态工具一样，它经常会造成漏报；
这些情况应当被发现但却被其忽略了。

* 动态强制实施工具：Clang 的 [Thread Sanitizer](https://clang.llvm.org/docs/ThreadSanitizer.html)（即 TSAN）
...
参见向某个未来的 TS 或 C++ 标准的修订版添加 `synchronized_value` [WG21 提案](https://wg21.link/p0290)。
...
因为相同的理由，这几乎同样强有力地适用于数据成员。
    ...
    class My_type {
        volatile int i = 0; // 可以的，volatile 数据成员
        // etc.
    };

##### 注解

与其他一些语言不同，C++ 中的 `volatile` [和同步没有任何关系](#rconc-volatile)。
...
按值返回，且移动比复制更廉价的局部变量，不应声明为 `const`，
因为这将强迫进行不必要的复制。

    std::vector<int> f(int i)
    {
        std::vector<int> v{ i, i, i };  // 不需要 const
        return v;
    }
...
* 开发一种概念识别工具（例如，参考[一种早期实验](https://www.stroustrup.com/sle2010_webversion.pdf)）。
...
[构造函数的模板形参推断(Rev. 3)](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0091r1.html)。
例如：
...
人们发现 `Link` 不再隐藏在列表中很可怕，所以我们命名这个技术为 [SCARY](https://www.open-std.org/jtc1/sc22/WG21/docs/papers/2009/n2911.pdf)。
...
* [SF.13: 在 `#include` 语句中使用可移植的标识符](#rs-portable-header-id)
* 
...
* [工作草案，C++ 的模块扩展](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/n4592.pdf)
* [模块，组件化及其迁移](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0141r0.pdf)
...
字符串字面量，则必须如此，而且根据[规则](https://eel.is/c++draft/over.literal)——用户必须以
...
[标准](https://eel.is/c++draft/cpp.include) 向编译器提供了对于实现
...
### <a name="Rs-portable-header-id"></a>SF.13: 在 `#include` 语句中使用可移植的标识符

##### 理由

[标准](https://eel.is/c++draft/cpp.include)并未指定编译器是怎样从 `#include` 指令中的标识符唯一定位头文件的，它也没有指定由什么构成唯一性。例如，实现是否将标识符视为大小写敏感，或标识符是否是到头文件的文件系统路径，以及若是如此，文件系统路径的层次是如何分隔的。
...
注意已经为 `string` 提供了 `>>` 和 `!=`（作为有用操作的例子），并且没有显式的内存分配，
...
* [Bjarne Stroustrup 的个人主页](https://www.stroustrup.com)
* [WG21](https://www.open-std.org/jtc1/sc22/wg21/)
* [Boost](https://www.boost.org)<a id="Boost"></a>
...
* ISO C++ Standard C++11.
* ISO C++ Standard C++14.
* [ISO C++ Standard C++17](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/n4606.pdf). 委员会草案。
...
* [WG21 Ranges report](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/n4569.pdf). 草案。
...
and here are the [slides]([https://2017.cppconf.ru/talks/sergey-zubkov](https://www.slideshare.net/slideshow/c-core-guidelines-72335317/72335317)). In Russian. 2017.
...
[坚持进行初始化](#res-always)。
* <a id="pro-type-memberinit"></a>Type.6: 坚持初始化数据成员：
[坚持进行初始化](#res-always)，
...
总之，请使用[通过表决进入 C++17 的版本](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0088r3.html)。
...
等待标准委员会对于契约和断言语法的确定。
参见使用属性语法的[契约提案](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0380r1.pdf)，
...
这些规则恐怕会和 [PPP Style Guide](https://www.stroustrup.com/Programming/PPP-style.pdf) 中的建议有很强的相似性，
它是为支持 Stroustrup 的 [Programming: Principles and Practice using C++](https://www.stroustrup.com/programming.html) 而编制的。
...
避免包含双下划线 `__` 或以一个下划线跟着一个大写字母开头（例如 `_Throws`）的标识符命名。
这种标识符为 C++ 实现所保留。
...
尤其是，这避免混淆宏和非宏符号常量（另见 [Enum.5: 请勿为枚举符采用 `ALL_CAPS` 命名方式](#renum-caps)）
...
这些编码指导方针使用的是 [CommonMark](https://commonmark.org)，以及 `<a>` HTML 锚定元素。
...
### <a id="sd-order"></a>讨论: 以成员的声明顺序进行数据成员的定义和初始化

数据成员总是以它们在类定义中的声明顺序进行初始化，因此在构造函数初始化列表中应当以该顺序来书写它们。以别的顺序书写它们只会让代码混淆，因为它并不会以你所见到的顺序来运行，而这会导致难于发现与顺序有关的 BUG。
...
如果类定义和构造函数体是在不同文件中的话，这种由数据成员声明顺序对构造函数的正确性造成的远距离影响将更难于发现。
...
如果定义了复制操作，且有任何基类或成员的类型定义了移动操作的话，应当同样定义移动操作。
...
### <a id="St-clangtidy"></a>工具: [Clang-tidy](https://clang.llvm.org/extra/clang-tidy/checks/list.html)
...
（基于 [Programming: Principles and Practice using C++](https://www.stroustrup.com/programming.html) 中的词汇表）。
...
* *ISO*: 国际标准化组织。C++ 语言是一项 ISO 标准：ISO/IEC 14882。更多信息请参考 [iso.org](https://iso.org)。
